		...
		return <div className="list_item">
				<div className="list_item_text_wrap">
					...
					<div className="list_item_info_text">
						...
						<ul className="grades">
							{this.state.expand && this.props.data.grades.length > 0 && this.props.data.grades.map((grade, ind) =>
									<li key={ind}>
										<span className="left">
											Test {ind + 1}
										</span>
						...
						<div className="tags">
							{ this.props.data.tags.length > 0 && this.props.data.tags.map((tag, ind)=> {
								return <button key={ind} className="btn">{ tag }</button>
							})}
						</div>
				...
				<div className="expand">
							<h1 onClick={this.toggle} className="expand_btn">{this.state.expand? "-" : "+"}</h1>
				</div>
			</div>;
	}
}
